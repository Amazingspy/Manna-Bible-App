...
const API_BASE = Platform.OS === 'android' ? 'http://10.0.2.2:3000/server/manna_function' : 'http://localhost:3000/server/manna_function';
...
// Separate component to handle the video hook lifecycle
const VideoPlayerComponent = ({ videoUrl }) => {
    ...
    return (
        <View style={{ flex: 1, backgroundColor: 'black' }}>
            ...
            />
            {/* Minimal Custom Control UI overlaid to indicate we're tracking state */}
            <View className="absolute bottom-4 right-4 bg-black/60 rounded-full px-4 py-1.5 border border-white/20">
                <Text className="text-white text-[10px] font-bold uppercase tracking-widest">{isPlaying ? 'Playing' : 'Paused'}</Text>
            </View>
        </View>
    );
};
...
export default function MediaScreen() {
    ...
    useEffect(() => {
        fetch(`${API_BASE}/videos`)
            ...
            .then(response => {
                ...
                // Build category list dynamically from folders if any
    ...
    const renderItem = ({ item }) => {
        ...
        return (
            <TouchableOpacity
                activeOpacity={0.9}
                className={`mx-6 mt-4 ${isActive ? 'mb-8' : 'mb-6'}`}
                onPress={() => setActiveVideoId(isActive ? null : item.id)}
            >
                {isActive ? (
                    <View className="h-56 w-full overflow-hidden rounded-[2rem] shadow-xl bg-black">
                        <VideoPlayerComponent videoUrl={item.url} />
                    </View>
                ) : (
    ...
    return (
        <SafeAreaView className="flex-1 bg-white dark:bg-background-dark pb-6">
            <View className="flex-1">
                {/* 
                    By placing the Header exactly here outside the FlatList, 
                    it will stay fixed at the top while the FlatList below it scrolls independently! 
                */}
                {renderHeader()}

                {/* Independently Scrollable List of Overviews */}
                <View className="flex-1 mt-4">
                    <Text className="px-6 mb-4 text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">
                        {selectedCategory === "All" ? "Bible Book Overviews" : `${selectedCategory} Videos`}
                    </Text>
                    <FlatList
                        data={filteredVideos}
